...
![The image shows a labeled diagram of a command line interface (CLI). Key parts of the CLI are annotated as follows: Command line prompt: Highlighted with a red bracket and arrow pointing to the text (base) ciprian ~$, indicating the prompt where users can type commands. Command to run: The command date is highlighted with an arrow, indicating that this is the instruction the user typed. Computer’s response: The output Wed Jul 31 16:47:09 EDT 2019 is labeled as the system's response to the date command. New line (can run new command here): The cursor after the next command prompt (base) ciprian ~$ is pointed out, indicating where the user can enter the next command.](/curriculum-assets/302/command-line-layout.png)

<center>
  <small>
    _Source: [Edlitera](https://www.edlitera.com/blog/posts/the-command-line-1)_
  </small>
</center>
...
We will use [the React repository on GitHub](https://github.com/facebook/react) as an exemplar going forward to demonstrate various features of git and GitHub, but feel free to peruse it on your own time as well.
...
- [Atlassian Git Basics](https://www.atlassian.com/git)
